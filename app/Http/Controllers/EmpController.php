...
class EmpController extends Controller
{
    ...
    public function processEmployee(Request $request)
    {
        ...
        //     'date_of_birth'     => date_format(date_create($request->dob), 'Y-m-d'),
        //     'date_of_joining'   =>date_format(date_create($request->doj), 'Y-m-d'),
        ...
        //     'date_of_confirmation'=>date_format(date_create($request->date_of_confirmation), 'Y-m-d'),
        ...
        //     'date_of_resignation'=>$request->date_of_resignation == '' ? null :  date_format(date_create($request->date_of_resignation), 'Y-m-d'),
        //     'notice_period'     =>$request->notice_period == '' ? null : $request->notice_period,
        //     'last_working_day'  =>$request->last_working_day == '' ? null : date_format(date_create($request->last_working_day), 'Y-m-d'),
        ...
        $kin = NextOfKin::create([
            ...
            'date_of_birth'=>date_format(date_create($request->kin_dob), 'Y-m-d'),
            'gender'=>$request->kin_gender,
            'employee_id'=> $emp->id 
        ]);
        // newNextOfKin::where('id', $kin->id)->update([
        //   'employee_id'=>$emp->id 
        // ]);
    ...
    public function updateEmployeePicure(Request $request){
        $rules = [
            'photo'=>'required'
        ];
        $data = $request->all();
        ...
        if($validator->fails()){
            \Session::flash('flash_message_error', 'Picture required');
            return redirect()->back()->with('message', 'Picture required');
        }
        $file             = $request->file('photo');
        $allowedExtension = ['jpg', 'jpeg', 'png'];
        $filename         = str_random(12);
        ...
        $destinationPath  = public_path('photos');
        if (!in_array($fileExt, $allowedExtension)) {
            \Session::flash('flash_message_error', 'Extension not allowed');
            return redirect()->back()->with('message', 'Extension not allowed');
        }
        $filename = $filename . '.' . $fileExt;
        $file->move($destinationPath, $filename); 
        $emp = Employee::where('id', $data['emp_id'])->first();
        $this->deleteProfileImage($emp->photo);
        Employee::where('id', $data['emp_id'])->update([
            'photo'=> $filename
        ]);
        \Session::flash('flash_message', 'Picture successfully updated');
    ...
    public function showEmployeeKin(){
        $kins = NextOfKin::all()->paginate(15);
        $column = '';
        $string = '';
        return view('hrms.employee.show_kins', compact('kins', 'column', 'string'));
    }
...
\Log::info($row->role);
    ...
    private function deleteProfileImage($path){
        $image_path = public_path('photos/').$path;
        unlink($image_path);
    }
}
